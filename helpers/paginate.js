...
exports.paginate = (totalItems, itemsPerPage, currentPage, url, param_name) => {
    ...
    html.push('<div class="pagination"><ul class="actions">');
    ...
    if (1 < currentPage - neighbours) {
        ...
        html.push('<li><a class="button small fit" href="' + url + '">' + 1 + '</a></li>');
    }
    ...
    if (currentPage - neighbours > 2) {
        ...
        html.push('<li><a class="button small fit" href="' + url + '">' + n + '</a></li>');
    }

    // Pages in the middle
    for (let i = 1; i <= total; i++) {
        if (i === currentPage) {
            html.push('<li><a class="button small fit alt" href="#">' + i + '</a></li>');
        } else {
            if (i >= currentPage - neighbours && i <= currentPage + neighbours) {
                url = addPagenoToUrl(url, i, param_name);
                html.push('<li><a class="button small fit" href="' + url + '">' + i + '</a></li>');
            }
        }
    }
    ...
    if (total > currentPage + neighbours) {
        ...
        html.push('<li><a class="button small fit" href="' + url + '">' + total + '</a></li>');
    }
